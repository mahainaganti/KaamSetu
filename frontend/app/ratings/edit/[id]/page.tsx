...
export default function EditRatingPage() {
  const params = useParams<{ id: string }>(); const router = useRouter(); const showToast = useToast(); const [formData, setFormData] = useState<RatingFormData | null>(null); const [bookings, setBookings] = useState<BookingOption[]>([]); const [employers, setEmployers] = useState<EmployerOption[]>([]); const [workers, setWorkers] = useState<WorkerOption[]>([]); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
